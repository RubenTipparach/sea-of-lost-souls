...
- **Desktop UI is integrated (in-canvas); the DOM is mobile-only.** The target
  experience is desktop (mouse + keyboard), and **desktop control surfaces
  must be rendered inside the wgpu canvas as integrated UI** (e.g. egui via
  `egui_wgpu`), not as separate HTML/DOM elements. This includes the build
  menu, Sensors / Build / Pause / Focus buttons, the stance dropdown, the MU /
  crew / queue readouts, and any future selection/command panels. The DOM
  overlay (`#sol-controls`, `#sol-build-overlay`, etc.) is a **mobile test
  harness** only: it exists to keep gameplay exercisable on a phone/tablet
  while we iterate, and it drives the *same* wasm-exported commands as the
  desktop path. The DOM overlay is hidden on desktop builds; the integrated
  UI is hidden on mobile. Every gameplay feature must remain reachable on
  both surfaces, but the desktop UI is the polished one and the mobile UI
  stays minimal.
